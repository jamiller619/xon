...
import { dataSources, groups, libraries, mediaItems, suggestedGroups, users } from "../schema.js";
...
describe("AI Suggested Groups API", () => {
  ...
  async function insertMediaItem(fileName: string, category: string, metadata = "{}") {
  ...
  describe("GET /api/v1/ai/suggested-groups", () => {
    it("returns pending suggestions for admin", async () => {
      await insertSuggestion({ suggestedTitle: "My Series", status: "pending" });
      await insertSuggestion({ suggestedTitle: "Accepted One", status: "accepted" });

      const res = await app.request("/api/v1/ai/suggested-groups", {
        headers: { Authorization: AUTH_ADMIN },
      });
      expect(res.status).toBe(200);
      const body = (await res.json()) as { items: { suggestedTitle: string }[] };
    ...
    it("supports status filter", async () => {
      ...
      const res = await app.request("/api/v1/ai/suggested-groups?status=accepted", {
        headers: { Authorization: AUTH_ADMIN },
      });
      expect(res.status).toBe(200);
      const body = (await res.json()) as { items: { suggestedTitle: string }[] };
    ...
    it("supports libraryId filter", async () => {
      ...
      const res = await app.request(`/api/v1/ai/suggested-groups?libraryId=${libraryId}`, {
        headers: { Authorization: AUTH_ADMIN },
      });
      expect(res.status).toBe(200);
  ...
  describe("POST /api/v1/ai/suggested-groups/scan", () => {
    it("returns found count of 0 for empty library", async () => {
      const res = await app.request("/api/v1/ai/suggested-groups/scan", {
        method: "POST",
        headers: { Authorization: AUTH_ADMIN, "Content-Type": "application/json" },
  ...
  describe("POST /api/v1/ai/suggested-groups/:id/accept", () => {
    it("accepts a pending suggestion and creates a group", async () => {
      ...
      const res = await app.request(`/api/v1/ai/suggested-groups/${suggId}/accept`, {
        method: "POST",
        headers: { Authorization: AUTH_ADMIN },
      });
      expect(res.status).toBe(200);
    ...
    it("returns 404 for unknown suggestion", async () => {
      const res = await app.request("/api/v1/ai/suggested-groups/nonexistent/accept", {
        method: "POST",
        headers: { Authorization: AUTH_ADMIN },
      });
      expect(res.status).toBe(404);
  ...
  describe("POST /api/v1/ai/suggested-groups/:id/reject", () => {
    it("rejects a pending suggestion", async () => {
      ...
      const res = await app.request(`/api/v1/ai/suggested-groups/${suggId}/reject`, {
        method: "POST",
        headers: { Authorization: AUTH_ADMIN },
      });
      expect(res.status).toBe(200);
    ...
    it("returns 404 for unknown suggestion", async () => {
      const res = await app.request("/api/v1/ai/suggested-groups/nonexistent/reject", {
        method: "POST",
        headers: { Authorization: AUTH_ADMIN },
      });
      expect(res.status).toBe(404);
