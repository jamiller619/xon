...
const MANUAL_GROUP_TYPES = ["collection", "playlist", "album", "shelf", "folder"] as const;
...
export function makeGroupsRouter(db: LibSQLDatabase): Hono {
  ...
  router.get("/", async (c) => {
    ...
    if (!libraryId) return c.json({ error: "libraryId query param required" }, 400);
    ...
    const rows = await db
      .select()
      .from(groups)
      .where(and(eq(groups.libraryId, libraryId), inArray(groups.type, [...MANUAL_GROUP_TYPES])))
  ...
  router.get("/:id", async (c) => {
    ...
    const membersWithThumbs = members.map((m) => {
      let thumbnailUrls: { small: string; medium: string; large: string } | null = null;
      if (m.thumbnailPaths) {
        try {
    ...
    async (c) => {
      ...
      for (const item of body.items) {
        await db
          ...
          .where(
            and(eq(groupMembers.groupId, groupId), eq(groupMembers.mediaItemId, item.mediaItemId))
          );
      }
  ...
  router.delete("/:id/items/:mediaItemId", requireRole("manager"), async (c) => {
    const groupId = c.req.param("id");
    ...
    await db
      .delete(groupMembers)
      .where(and(eq(groupMembers.groupId, groupId), eq(groupMembers.mediaItemId, mediaItemId)));
