...
import { backupFileState, backupTargets, backupVerifyJobs, users } from "../schema.js";
import { computeChecksum } from "./adminBackupVerify.js";
...
describe("Admin Backup Verify API", () => {
  ...
  describe("POST /api/v1/admin/backup/verify/:targetId", () => {
    ...
    it("returns 404 for unknown target", async () => {
      const res = await app.request("/api/v1/admin/backup/verify/nonexistent-id", {
        method: "POST",
        headers: { Authorization: ADMIN_AUTH },
      });
    ...
    it("returns 400 for disabled target", async () => {
      await db
        ...
        .set({ enabled: false })
        .where((await import("drizzle-orm").then((m) => m.eq))(backupTargets.id, targetId));
      const res = await app.request(`/api/v1/admin/backup/verify/${targetId}`, {
        method: "POST",
        headers: { Authorization: ADMIN_AUTH },
      });
  ...
  describe("GET /api/v1/admin/backup/verify/jobs/:id", () => {
    it("returns 404 for unknown job id", async () => {
      const res = await app.request("/api/v1/admin/backup/verify/jobs/no-such-id", {
        headers: { Authorization: ADMIN_AUTH },
      });
      expect(res.status).toBe(404);
    });
    ...
    it("returns job record by id", async () => {
      const createRes = await app.request(`/api/v1/admin/backup/verify/${targetId}`, {
        method: "POST",
        headers: { Authorization: ADMIN_AUTH },
      });
      ...
      const res = await app.request(`/api/v1/admin/backup/verify/jobs/${jobId}`, {
        headers: { Authorization: ADMIN_AUTH },
      });
      expect(res.status).toBe(200);
      const body = await res.json();
  ...
  describe("runVerifyJob", () => {
    it("marks job completed with all passed when checksums match", async () => {
      ...
      const jobRows = await db
        ...
        .where((await import("drizzle-orm").then((m) => m.eq))(backupVerifyJobs.id, jobId));
      const job = jobRows[0];
