...
import { dataSources, libraries, mediaItems, users } from "../schema.js";
  ...
  describe("PUT /api/v1/media/:id/progress", () => {
    ...
    it("updates progress on subsequent calls", async () => {
      ...
      const res = await app.request("/api/v1/media/item-1/progress", {
        method: "PUT",
        headers: { "Content-Type": "application/json", Authorization: USER_AUTH },
        body: JSON.stringify({ position: 300, duration: 3600, completed: false }),
      });
      expect(res.status).toBe(200);
    });
    ...
    it("can mark item as completed", async () => {
      const res = await app.request("/api/v1/media/item-1/progress", {
        method: "PUT",
        headers: { "Content-Type": "application/json", Authorization: USER_AUTH },
        body: JSON.stringify({ position: 3600, duration: 3600, completed: true }),
      });
      expect(res.status).toBe(200);
    });
