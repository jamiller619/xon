...
import { syncProfiles, syncRuns } from "../schema.js";
...
describe("Sync Profiles API", () => {
  ...
  it("POST /sync/profiles — creates a partial sync profile", async () => {
    ...
    expect(JSON.parse(body.scope)).toEqual({ libraryIds: ["lib-1"], mediaTypes: ["Movies"] });
  ...
  it("PUT /sync/profiles/:id — updates fields", async () => {
    ...
    const res = await app.request(`/api/v1/sync/profiles/${created.id}`, {
      method: "PUT",
      headers: { Authorization: AUTH, "Content-Type": "application/json" },
      body: JSON.stringify({ name: "New Name", targetPath: "/sync/new", includeMedia: true }),
    });
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("POST /sync/profiles/:id/run — writes metadata.json to targetPath", async () => {
    ...
    expect(vi.mocked(mkdir)).toHaveBeenCalledWith("/sync/meta", { recursive: true });
  ...
  it("GET /sync/runs/:id — returns a single run", async () => {
    ...
    const runRes = await app.request(`/api/v1/sync/profiles/${profile.id}/run`, {
      method: "POST",
      headers: { Authorization: AUTH },
    });
    const runBody = await runRes.json();

    const res = await app.request(`/api/v1/sync/profiles/runs/${runBody.runId}`, {
      headers: { Authorization: AUTH },
    });
    expect(res.status).toBe(200);
    const body = await res.json();
