...
import { backupJobs, backupTargets, dataSources, libraries, mediaItems, users } from "../schema.js";
...
describe("Admin Backup Media API", () => {
  ...
  describe("GET /api/v1/admin/backup/media/jobs/:id", () => {
    it("returns specific job by id", async () => {
      ...
      const res = await app.request(`/api/v1/admin/backup/media/jobs/${jobId}`, {
        headers: { Authorization: ADMIN_AUTH },
      });
      expect(res.status).toBe(200);
      const body = await res.json();
    ...
    it("returns 404 for unknown job id", async () => {
      const res = await app.request("/api/v1/admin/backup/media/jobs/nonexistent", {
        headers: { Authorization: ADMIN_AUTH },
      });
      expect(res.status).toBe(404);
    });
  ...
  describe("backup job execution", () => {
    ...
    it("marks job failed when target not found", async () => {
      ...
      await db.delete(backupTargets).where(eqDrizzle(backupTargets.id, badTargetId));
