  ...
  return {
    default: {
      ...
      crypto: {
        ...
        createCsr: vi.fn().mockResolvedValue([Buffer.from("domain-key"), Buffer.from("csr-data")]),
      },
    },
  };
});
...
import { acquireAcmeCert, isCertFileValid, loadManualCerts } from "./httpsManager.js";
...
describe("loadManualCerts", () => {
  it("reads cert and key from disk", async () => {
    ...
    const result = await loadManualCerts("/path/to/cert.pem", "/path/to/key.pem");
  ...
  it("propagates read errors", async () => {
    ...
    await expect(loadManualCerts("/no/cert", "/no/key")).rejects.toThrow("ENOENT");
  });
});
...
describe("acquireAcmeCert", () => {
  beforeEach(() => {
    ...
    vi.mocked(acme.crypto.createPrivateKey).mockResolvedValue(Buffer.from("acct-key") as never);
