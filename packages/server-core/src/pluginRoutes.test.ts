...
import type { PluginContext, PluginManifest, RouteDefinition } from "@xon/plugin-sdk";
...
describe("pluginRouteDispatcher — multiple plugins", () => {
  it("routes are isolated per plugin namespace", async () => {
    ...
    makeActiveEntry(
      [{ method: "GET", path: "/status", handler: (c) => c.json({ plugin: "test" }) }],
      baseManifest
    );
    ...
    _registerPlugin({
      ...
      status: "active",
      hooks: [],
      routes: [{ method: "GET", path: "/status", handler: (c) => c.json({ plugin: "other" }) }],
      uiComponents: [],
    });
...
describe("pluginRouteDispatcher — context registered via activatePlugin", () => {
  it("routes registered during activate() are served", async () => {
    ...
    const { activatePlugin } = await import("./pluginManager.js");
