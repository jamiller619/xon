...
const ROLE_RANK: Record<string, number> = { guest: 0, user: 1, manager: 2, admin: 3 };
 ...
 */
export function requireRole(minRole: "user" | "manager" | "admin"): MiddlewareHandler {
  return async (c: Context, next: Next) => {
    const user = c.get("user");
    const userRank = ROLE_RANK[user?.role ?? ""] ?? -1;
    const minRank = ROLE_RANK[minRole] ?? 99;
    if (userRank < minRank) {
      return c.json({ error: `Forbidden: requires ${minRole} role or higher` }, 403);
    }
    return next();
  };
}
