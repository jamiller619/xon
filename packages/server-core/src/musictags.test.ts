...
import { type MusicTagsMetadata, extractMusicTags, isMusicCategory } from "./musictags.js";
...
type MockCommon = {
  ...
  picture?: { format: string; data: Uint8Array; type?: string; description?: string }[];
};
...
describe("extractMusicTags", () => {
      ...
      makeMeta(
        {
          ...
          track: { no: 11, of: 12 },
          disk: { no: 1, of: 1 },
          picture: [{ format: "image/jpeg", data: new Uint8Array([0xff, 0xd8]) }],
        },
        {
          duration: 354.32,
          bitrate: 320000,
  ...
  it("returns empty object for file with no tags", async () => {
    mockParseFile.mockResolvedValue(
      makeMeta({ track: { no: null, of: null }, disk: { no: null, of: null } }, {}) as never
    );
  ...
  it("takes first genre when multiple genres are present", async () => {
    mockParseFile.mockResolvedValue(makeMeta({ genre: ["Jazz", "Blues", "Soul"] }) as never);
  ...
  it("returns null when parseFile throws (file not found)", async () => {
    mockParseFile.mockRejectedValue(new Error("ENOENT: no such file or directory"));
  ...
  it("extracts OGG Vorbis metadata", async () => {
    mockParseFile.mockResolvedValue(
      makeMeta(
        ...
        { duration: 180.0, sampleRate: 48000, numberOfChannels: 2, codec: "Vorbis" }
      ) as never
    );
