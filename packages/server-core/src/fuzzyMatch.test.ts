...
describe("parseFilenameInfo", () => {
  ...
  it("strips quality tags from filename", () => {
    const { title } = parseFilenameInfo("The.Dark.Knight.2008.1080p.BluRay.mkv");
...
describe("matchMediaFile", () => {
  ...
  it("returns results sorted by confidence descending", async () => {
    ...
    for (let i = 0; i < results.length - 1; i++) {
      expect(results[i]?.confidence).toBeGreaterThanOrEqual(results[i + 1]?.confidence ?? 0);
    }
  });
  ...
  it("marks local results with source=local", async () => {
    const results = await matchMediaFile("Inception.mkv", "/movies/Inception.mkv", {}, candidates);
  ...
  it("calls cloud API when best local confidence is below threshold", async () => {
    ...
    try {
      const results = await matchMediaFile("xxxxunknown.mkv", "/movies/xxxxunknown.mkv", {}, [], {
  ...
  it("does not call cloud API when best local confidence is above threshold", async () => {
    ...
    try {
      await matchMediaFile("Inception.mkv", "/movies/Inception.mkv", {}, [{ title: "Inception" }], {
  ...
  it("returns empty array when no candidates and no cloud config", async () => {
    const results = await matchMediaFile("unknown.mkv", "/movies/unknown.mkv", {}, []);
