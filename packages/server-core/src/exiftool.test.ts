...
import { type ExiftoolMetadata, extractExiftoolMetadata, isImageCategory } from "./exiftool.js";
...
describe("extractExiftoolMetadata", () => {
  ...
  it("returns null when exiftool is not installed (ENOENT)", async () => {
    ...
    const err = Object.assign(new Error("spawn exiftool ENOENT"), { code: "ENOENT" });
    proc.emit("error", err);
