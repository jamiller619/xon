...
import { RAW_EXTENSIONS, convertRawToJpeg, isRawImage } from "./raw.js";

type FakeChildProcess = EventEmitter & { stdout: EventEmitter; stderr: EventEmitter };
...
describe("RAW_EXTENSIONS", () => {
  it("contains required RAW formats", () => {
    for (const ext of [".cr2", ".cr3", ".nef", ".arw", ".dng", ".orf", ".raf"]) {
...
describe("convertRawToJpeg", () => {
  it("returns embedded JPEG preview from dcraw -c -e (fast path)", async () => {
    ...
    expect(mockSpawn).toHaveBeenCalledWith("dcraw", ["-c", "-e", "/photos/shot.cr2"]);
  });
  ...
  it("falls back to full decode when embedded preview fails, then converts TIFF via sharp", async () => {
    ...
    mockSharp.mockReturnValue({ jpeg: jpegMock } as unknown as ReturnType<typeof sharpFn>);

    const result = await convertRawToJpeg("/photos/shot.nef");
    expect(result).toEqual(jpegData);
    expect(mockSpawn).toHaveBeenCalledTimes(2);
    expect(mockSpawn).toHaveBeenNthCalledWith(1, "dcraw", ["-c", "-e", "/photos/shot.nef"]);
    expect(mockSpawn).toHaveBeenNthCalledWith(2, "dcraw", ["-c", "-w", "-T", "/photos/shot.nef"]);
  ...
  it("throws with clear message when dcraw is not installed (ENOENT)", async () => {
    ...
    mockSpawn.mockImplementation(() => {
      Promise.resolve().then(() => {
        const err = Object.assign(new Error("spawn dcraw ENOENT"), { code: "ENOENT" });
        proc.emit("error", err);
      });
      return proc as unknown as ChildProcess;
    });

    await expect(convertRawToJpeg("/photos/shot.arw")).rejects.toThrow("dcraw not found");
  ...
  it("throws when dcraw fails on full decode after embedded preview fails", async () => {
    ...
    mockSpawn.mockImplementation(() => {
      ...
      Promise.resolve().then(() => {
        if (current === 2) proc.stderr.emit("data", Buffer.from("decode error"));
        proc.emit("close", current === 1 ? 1 : 2);
      });
      return proc as unknown as ChildProcess;
    });

    await expect(convertRawToJpeg("/photos/shot.orf")).rejects.toThrow("dcraw failed");
  });
});
