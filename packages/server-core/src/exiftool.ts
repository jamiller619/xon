...
export async function extractExiftoolMetadata(filePath: string): Promise<ExiftoolMetadata | null> {
  return new Promise((resolve) => {
    ...
    proc.on("close", (code: number | null) => {
      ...
      try {
        ...
        if (!Number.isNaN(lat) && data.GPSLatitude !== undefined) result.gpsLatitude = lat;

        const lon = Number(data.GPSLongitude);
        if (!Number.isNaN(lon) && data.GPSLongitude !== undefined) result.gpsLongitude = lon;
