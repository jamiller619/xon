...
import { createSandboxedFetch, createSandboxedFs, isPathAllowed } from "./pluginSandbox.js";
...
describe("isPathAllowed", () => {
  it("returns true for exact directory match", () => {
    expect(isPathAllowed("/plugins/my-plugin", ["/plugins/my-plugin"])).toBe(true);
  });

  it("returns true for a subpath within an allowed directory", () => {
    expect(isPathAllowed("/plugins/my-plugin/data/file.txt", ["/plugins/my-plugin"])).toBe(true);
  });

  it("returns true when path matches one of multiple allowed directories", () => {
    expect(isPathAllowed("/data/cache/result.json", ["/plugins/my-plugin", "/data/cache"])).toBe(
      true
    );
  });
  ...
  it("prevents traversal via ../ by resolving before comparing", () => {
    ...
    expect(isPathAllowed("/plugins/my-plugin/../../../etc/passwd", ["/plugins/my-plugin"])).toBe(
      false
    );
  });
  ...
  it("returns false for a path that is a prefix but not a subdir", () => {
    ...
    expect(isPathAllowed("/plugins/my-plugin-evil/secret", ["/plugins/my-plugin"])).toBe(false);
  });
...
describe("createSandboxedFs", () => {
  ...
  describe("readFile", () => {
    ...
    it("throws for traversal attempt via ../", async () => {
      ...
      await expect(sandboxedFs.readFile(`${pluginDir}/../../../etc/passwd`)).rejects.toThrow(
  ...
  describe("writeFile", () => {
    it("allows writeFile within pluginDir", async () => {
      ...
      expect(mockFs.writeFile).toHaveBeenCalledWith(`${pluginDir}/output.json`, "{}");
    });

    it("throws for writeFile outside allowed paths", async () => {
      const sandboxedFs = createSandboxedFs(pluginId, pluginDir, []);

      await expect(sandboxedFs.writeFile("/tmp/evil.sh", "rm -rf /")).rejects.toThrow(
  ...
  describe("readdir", () => {
    ...
    it("throws for readdir outside allowed paths", async () => {
      ...
      await expect(sandboxedFs.readdir("/etc")).rejects.toThrow("Filesystem access denied");
  ...
  describe("stat", () => {
    it("allows stat within pluginDir and returns result", async () => {
      const fakeStat = { size: 1024, isFile: () => true, isDirectory: () => false };
    ...
    it("throws for stat outside allowed paths", async () => {
      ...
      await expect(sandboxedFs.stat("/etc/shadow")).rejects.toThrow("Filesystem access denied");
  ...
  describe("mkdir", () => {
    ...
    it("allows mkdir within pluginDir with recursive option", async () => {
      ...
      expect(mockFs.mkdir).toHaveBeenCalledWith(`${pluginDir}/nested/deep`, { recursive: true });
    });

    it("throws for mkdir outside allowed paths", async () => {
      const sandboxedFs = createSandboxedFs(pluginId, pluginDir, []);

      await expect(sandboxedFs.mkdir("/tmp/hack")).rejects.toThrow("Filesystem access denied");
  ...
  describe("unlink", () => {
    ...
    it("throws for unlink outside allowed paths", async () => {
      ...
      await expect(sandboxedFs.unlink("/etc/crontab")).rejects.toThrow("Filesystem access denied");
...
describe("createSandboxedFetch", () => {
  ...
  it("passes RequestInit through to fetch", async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith("https://api.example.com/post", init);
  });
  ...
  it("throws for an invalid URL", async () => {
    ...
    await expect(sandboxedFetch("not-a-url")).rejects.toThrow("Network access denied");
    expect(globalThis.fetch).not.toHaveBeenCalled();
  });
