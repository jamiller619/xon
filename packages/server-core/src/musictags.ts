...
const MUSIC_CATEGORIES = new Set<string>([MediaCategory.Music, MediaCategory.Audiobooks]);
...
export async function extractMusicTags(filePath: string): Promise<MusicTagsMetadata | null> {
  try {
    ...
    if (Array.isArray(common.picture) && common.picture.length > 0) result.hasAlbumArt = true;
    ...
    if (format.numberOfChannels != null) result.channels = format.numberOfChannels;
    if (typeof format.codec === "string") result.codec = format.codec;

    return result;
  } catch (err) {
    console.error(`Music tag extraction failed for ${filePath}: ${(err as Error).message}`);
    return null;
  }
}
