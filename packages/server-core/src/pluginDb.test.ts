...
import { createPluginDatabaseAccess, validatePluginSql } from "./pluginDb.js";

// ─── validatePluginSql ───────────────────────────────────────────────────────

describe("validatePluginSql", () => {
  it("allows SELECT on any table", () => {
    expect(() => validatePluginSql("my-plugin", "SELECT * FROM libraries")).not.toThrow();
    expect(() =>
  ...
  it("allows SELECT case-insensitively", () => {
    expect(() => validatePluginSql("my-plugin", "select * from libraries")).not.toThrow();
    expect(() => validatePluginSql("my-plugin", "Select id From media_items")).not.toThrow();
  });

  it("allows PRAGMA statements", () => {
    expect(() => validatePluginSql("my-plugin", "PRAGMA table_info(libraries)")).not.toThrow();
  });

  it("allows EXPLAIN statements", () => {
    expect(() => validatePluginSql("my-plugin", "EXPLAIN SELECT * FROM media_items")).not.toThrow();
  });
  ...
  it("rejects CREATE TABLE without plugin prefix", () => {
    expect(() => validatePluginSql("my-plugin", "CREATE TABLE custom_data (id TEXT)")).toThrow(
      'must be prefixed with "plugin_my_plugin_"'
    );
  });
  ...
  it("rejects CREATE TABLE on core table", () => {
    ...
    expect(() => validatePluginSql("my-plugin", "CREATE TABLE libraries (id TEXT)")).toThrow(
      'write access denied to core table "libraries"'
    );
  });
  ...
  it("rejects INSERT INTO core table", () => {
    expect(() => validatePluginSql("my-plugin", "INSERT INTO media_items (id) VALUES (?)")).toThrow(
      'write access denied to core table "media_items"'
    );
  });

  it("rejects INSERT INTO another plugin's table", () => {
    expect(() =>
      validatePluginSql("plugin-a", "INSERT INTO plugin_plugin_b_data (x) VALUES (1)")
    ).toThrow('must be prefixed with "plugin_plugin_a_"');
  });

  it("allows UPDATE on plugin-prefixed table", () => {
    expect(() =>
      validatePluginSql("my-plugin", "UPDATE plugin_my_plugin_settings SET value = ? WHERE key = ?")
    ).not.toThrow();
  });

  it("rejects UPDATE on core table", () => {
    expect(() =>
      validatePluginSql("my-plugin", "UPDATE libraries SET name = ? WHERE id = ?")
    ).toThrow('write access denied to core table "libraries"');
  });

  it("allows DELETE FROM plugin-prefixed table", () => {
    expect(() =>
      validatePluginSql("my-plugin", "DELETE FROM plugin_my_plugin_settings WHERE key = ?")
    ).not.toThrow();
  });

  it("rejects DELETE FROM core table", () => {
    expect(() => validatePluginSql("my-plugin", "DELETE FROM data_sources WHERE id = ?")).toThrow(
      'write access denied to core table "data_sources"'
    );
  });

  it("allows DROP TABLE on plugin-prefixed table", () => {
    expect(() =>
      validatePluginSql("my-plugin", "DROP TABLE IF EXISTS plugin_my_plugin_cache")
    ).not.toThrow();
  });

  it("rejects DROP TABLE on core table", () => {
    expect(() => validatePluginSql("my-plugin", "DROP TABLE media_items")).toThrow(
      'write access denied to core table "media_items"'
    );
  });

  it("rejects unsupported statements", () => {
    expect(() => validatePluginSql("my-plugin", "ATTACH DATABASE 'other.db' AS other")).toThrow(
  ...
  it("converts plugin ID hyphens to underscores for prefix", () => {
    ...
    expect(() =>
      validatePluginSql("my-cool-plugin", "CREATE TABLE plugin_my_cool_plugin_data (id TEXT)")
    ).not.toThrow();
    expect(() =>
      validatePluginSql("my-cool-plugin", "CREATE TABLE plugin_mycoolplugin_data (id TEXT)")
  ...
  it("isolates plugins from each other", () => {
    ...
    expect(() =>
      validatePluginSql("plugin-b", "INSERT INTO plugin_plugin_b_data (x) VALUES (1)")
    ).not.toThrow();
  });
});
...
describe("createPluginDatabaseAccess", () => {
  async function setup() {
    ...
    `);
    await client.execute("INSERT INTO libraries (id, name) VALUES ('lib-1', 'My Library')");
    return { client, db };
  }
  ...
  it("passes query parameters to the statement", async () => {
    ...
    const rows = await access.query("SELECT id FROM libraries WHERE id = ?", ["lib-1"]);
    expect(rows).toHaveLength(1);

    const empty = await access.query("SELECT id FROM libraries WHERE id = ?", ["nonexistent"]);
  ...
  it("allows plugin to create its own table and insert/query data", async () => {
    ...
    await access.query("INSERT INTO plugin_my_plugin_prefs (key, value) VALUES (?, ?)", [
      "theme",
      "dark",
    ]);

    const rows = await access.query("SELECT key, value FROM plugin_my_plugin_prefs");
    expect(rows).toHaveLength(1);
  ...
  it("throws synchronously when SQL is not permitted (before executing)", async () => {
    ...
    await expect(
      access.query("INSERT INTO libraries (id, name) VALUES (?, ?)", ["x", "y"])
  ...
  it("plugin A cannot write to plugin B's table", async () => {
    ...
    await accessB.query("CREATE TABLE IF NOT EXISTS plugin_plugin_b_data (x TEXT)");

    // Plugin A should not be able to write to plugin B's table
    await expect(
      accessA.query("INSERT INTO plugin_plugin_b_data (x) VALUES (?)", ["stolen"])
    ...
    await expect(accessA.query("SELECT x FROM plugin_plugin_b_data")).resolves.toEqual([]);
  });
  ...
  it("returns empty array for statements that produce no rows", async () => {
    ...
    await access.query("CREATE TABLE IF NOT EXISTS plugin_my_plugin_log (msg TEXT)");
    const result = await access.query("INSERT INTO plugin_my_plugin_log (msg) VALUES (?)", [
      "hello",
    ]);
