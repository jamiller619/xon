  ...
  it("all extensions start with a dot and are lowercase", () => {
    for (const [, info] of Object.entries(CATEGORY_DEFINITIONS)) {
      for (const ext of info.extensions) {
        ...
        expect(ext, `Extension "${ext}" must be lowercase`).toBe(ext.toLowerCase());
      }
    }
  });
});
...
describe("getMediaCategory", () => {
  ...
  it("identifies category from full file path", () => {
    expect(getMediaCategory("/home/user/videos/movie.mkv")).toBe(MediaCategory.Movies);
    expect(getMediaCategory("/music/library/artist/album/track.flac")).toBe(MediaCategory.Music);
  });
