  ...
  const router = new Hono()
    ...
    .get('/:id', async (c) => {
      ...
      const collectionIds = user
        ...
        : []
      const result = { ...data, collectionIds }
      const etag = computeETag([data.updatedAt?.getTime(), collectionIds])
    ...
    .get('/:id/images/:kind/:index', async (c) => {
      const id = c.req.param('id')
    ...
    .post('/:id/images/posters/find', async (c) => {
      ...
      if (!item.matchId) {
        return c.json(
          { error: 'This title does not have a metadata match' },
          409,
        )
      }
    ...
    .get('/:id/stream', async (c) => {
      ...
      if (isRawImage(sourcePath)) {
        ...
        try {
          ...
          const msg =
            err instanceof Error ? err.message : 'RAW conversion failed'
