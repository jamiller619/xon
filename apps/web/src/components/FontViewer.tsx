...
export default function FontViewer({ mediaId, title, onClose }: Props) {
  ...
  const [specimenSize, setSpecimenSize] = useState<(typeof SPECIMEN_SIZES)[number]>(36);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const fontFace = new FontFace(fontFamily, `url(/api/v1/media/${mediaId}/stream)`);
        await fontFace.load();
        if (cancelled) return;
        ...
        <aside className={styles.sidebar ?? ""}>
          ...
          {meta && (
            <dl className={styles.metaList ?? ""}>
              ...
              <dt>Glyphs</dt>
              <dd>{meta.glyphCount != null ? meta.glyphCount.toLocaleString() : "—"}</dd>
