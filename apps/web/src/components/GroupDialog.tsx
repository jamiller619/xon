...
export type ManualGroupType = "collection" | "playlist" | "album" | "shelf" | "folder";
...
function inferGroupType(mediaCategories: string[]): ManualGroupType {
  ...
  const DOC_CATS = ["Documents", "Web Media", "Design Files", "Fonts", "Icons", "3D Models"];
...
const ALL_TYPES: ManualGroupType[] = ["collection", "playlist", "album", "folder", "shelf"];
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await apiFetch("/api/v1/groups", {
        ...
        body: JSON.stringify({ libraryId, type: groupType, title: title.trim() }),
      });
      if (!res.ok) {
      ...
      const group = (await res.json()) as { id: string; title: string; type: string };
      onCreated(group);
    } catch {
