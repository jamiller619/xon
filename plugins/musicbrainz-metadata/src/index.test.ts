...
describe("parseMusicPath", () => {
  ...
  it("parses Artist/Album/Title without track number", () => {
    const result = parseMusicPath("/music/Radiohead/OK Computer/Karma Police.mp3");
...
describe("MusicBrainzClient", () => {
  describe("searchRecording", () => {
    it("returns null when no recordings found", async () => {
      const fetch = makeFetch([{ ok: true, json: { count: 0, recordings: [] } }]);
    ...
    it("maps recording to MusicBrainzMetadata", async () => {
      const fetch = makeFetch([{ ok: true, json: { count: 1, recordings: [sampleRecording] } }]);
      const client = new MusicBrainzClient(fetch);
    ...
    it("sets isCompilation for Various Artists", async () => {
      const vaRecording = {
        ...
        "artist-credit": [
          {
            artist: { id: "art-va", name: "Various Artists", "sort-name": "Various Artists" },
            name: "Various Artists",
            joinphrase: "",
          },
        ],
      };
      const fetch = makeFetch([{ ok: true, json: { count: 1, recordings: [vaRecording] } }]);
      const client = new MusicBrainzClient(fetch);
...
describe("MusicBrainzMetadataPlugin", () => {
  it("creates the tracks table on init", async () => {
    ...
    expect(sqls.some((s) => s.includes("plugin_musicbrainz_metadata_tracks"))).toBe(true);
  });
  ...
  it("registers media:created and media:updated hooks", async () => {
    ...
    const ctx = makeContext();
    await plugin.init(ctx);
    const events = ((ctx.on as ReturnType<typeof vi.fn>).mock.calls as [string][]).map((c) => c[0]);
  ...
  it("registers a GET /metadata/:mediaId route", async () => {
    ...
    expect(calls.some((c) => c[0].method === "GET" && c[0].path === "/metadata/:mediaId")).toBe(
      true
    );
  });
  ...
  it("enriches a music track on media:created event", async () => {
    const fetchMock = makeFetch([
      ...
      // fetchReleaseDetails
      {
        ...
        json: {
          ...
          "label-info": [{ label: { id: "l1", name: "EMI" }, "catalog-number": "EMC 103" }],
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(expect.stringContaining("Bohemian Rhapsody"));
  });

  it("logs warning when no MusicBrainz match found", async () => {
    const fetchMock = makeFetch([{ ok: true, json: { count: 0, recordings: [] } }]);
    const plugin = new MusicBrainzMetadataPlugin();
    const ctx = makeContext({ fetch: fetchMock });
    await plugin.init(ctx);
    ...
    await handler?.({ mediaId: "m2", filePath: "/music/UnknownArtist9999/Unknown.mp3" });
  ...
  it("returns stored metadata from route handler", async () => {
    ...
    expect(mockC.json).toHaveBeenCalledWith(
      expect.objectContaining({ title: "Bohemian Rhapsody", isCompilation: false })
    );
  });
  ...
  it("returns 404 from route when no metadata stored", async () => {
    ...
    const handler = routeCalls.find((c) => c[0].path === "/metadata/:mediaId")?.[0]?.handler;
